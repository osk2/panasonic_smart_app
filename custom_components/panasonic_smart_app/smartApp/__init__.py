...
from .const import BASE_URL, APP_TOKEN
from . import urls
...
class SmartApp(object):
    ...
    def login(self):
        data = {"MemId": self.account, "PW": self.password, "AppToken": self.app_token}
        response = requests.post(urls.login(), json=data)
        response.raise_for_status()

        self.CPToken = response.json().get("CPToken")
        self.header = {"Content-Type": "application/json", "CPToken": self.CPToken}
